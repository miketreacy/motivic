...
function _getMelodyTimeDuration(melody) {
    return melody.notes.reduce((time, note) => {
        return time + _getNoteTimeDuration(note.duration, melody.bpm, melody.timeSignature);
    }, 0);
}
 ...
 */
export function playMelody(melody, time, voice, stopCallback = Function.prototype) {
    ...
    for (let i = 0; i < melody.notes.length; i++) {
        time = playTone(ctx, melody.notes[i], time, melody.bpm, melody.timeSignature, voice);
    }
    setTimeout(() => {
        setPlayState(false);
        stopCallback();
...
export function loopMelody(melody, time, voice) {
    ...
    if (getPlayState()) {
        ...
        ctx.timeoutIDs.push(setTimeout(() => {
